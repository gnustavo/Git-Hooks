...
# Enable plugin
$clone->run(config => 'githooks.plugin', 'CheckAcls');
...
$clone->run(config => 'githooks.userenv', 'ACL_ADMIN');
$clone->run(config => 'githooks.admin', 'admin');
...
$clone->run(config => '--replace-all', 'githooks.admin', '^adm');
check_can_push('allow if admin matches regex');

$clone->run(config => '--replace-all', 'githooks.userenv', 'eval:x y z');
check_cannot_push('disallow if userenv cannot eval', 'master', qr/error evaluating userenv value/);

$clone->run(config => '--replace-all', 'githooks.userenv', 'eval:"nouser"');
check_cannot_push('disallow if userenv eval to nouser');

$clone->run(config => '--replace-all', 'githooks.userenv', 'eval:$ENV{ACL_ADMIN}');
check_can_push('allow if userenv can eval');

# Configure groups
$clone->run(config => 'githooks.groups', <<'EOF');
admins1 = admin
admins = @admins1
EOF

$clone->run(config => '--replace-all', 'githooks.admin', '@admins');
check_can_push('allow if admin in group');

$clone->run(config => '--unset', 'githooks.admin');

$clone->run(config => 'githooks.checkacls.acl', 'admin U master');
check_cannot_push('deny ACL master');

$clone->run(config => '--replace-all', 'githooks.checkacls.acl', 'admin U refs/heads/master');
check_can_push('allow ACL refs/heads/master');

$clone->run(config => '--replace-all', 'githooks.checkacls.acl', 'admin U refs/heads/branch');
check_cannot_push('deny ACL other ref');

$clone->run(config => '--replace-all', 'githooks.checkacls.acl', 'admin U ^.*/master');
check_can_push('allow ACL regex ref');

$clone->run(config => '--replace-all', 'githooks.checkacls.acl', 'admin U !master');
check_cannot_push('deny ACL negated regex ref');

$clone->run(config => '--replace-all', 'githooks.checkacls.acl', '^adm U refs/heads/master');
check_can_push('allow ACL regex user');

delete $ENV{VAR};
$clone->run(config => '--replace-all', 'githooks.checkacls.acl', '^adm U refs/heads/{VAR}');
...
$clone->run(config => '--replace-all', 'githooks.checkacls.acl', '@admins U refs/heads/master');
check_can_push('allow ACL user in group ');

$clone->run(config => '--replace-all', 'githooks.checkacls.acl', 'admin DUR refs/heads/fix');
$repo->run(checkout => '-q', '-b', 'fix');
check_cannot_push('deny ACL create ref', 'heads/fix');

$clone->run(config => '--replace-all', 'githooks.checkacls.acl', 'admin C refs/heads/fix');
check_can_push('allow create ref', 'heads/fix');

$repo->run(checkout => '-q', 'master');
$repo->run(branch => '-D', 'fix');

check_cannot_push('deny ACL delete ref', ':refs/heads/fix');

$clone->run(config => '--replace-all', 'githooks.checkacls.acl', 'admin D refs/heads/fix');
...
$clone->run(config => '--replace-all', 'githooks.checkacls.acl', 'admin CDU refs/heads/master');
$repo->run(reset => '--hard', 'HEAD~2'); # rewind fix locally
check_cannot_push('deny ACL rewrite ref', '+master:master'); # try to push it

$clone->run(config => '--replace-all', 'githooks.checkacls.acl', 'admin R refs/heads/master');
check_can_push('allow ACL rewrite ref', '+master:master'); # try to push it

$clone->run(config => '--replace-all', 'githooks.checkacls.acl', 'admin CRUD refs/heads/master');
$repo->run(tag => '-a', '-mtag', 'objtag'); # object tag
check_cannot_push('deny ACL push tag');

$clone->run(config => '--add', 'githooks.checkacls.acl', 'admin CRUD ^refs/tags/');
