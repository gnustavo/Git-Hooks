...
sub check_can_commit {
    ...
    test_ok($testname, $repo, 'commit', '-F', $msgfile,
            defined $author && '--author', defined $author && $author);
}
...
# No limits
check_can_commit('This author can commit (1): pattern', 'Dummy commit message',
        'A UThor <a.uthor@site>');

# Limit by matching pattern.
$repo->command(config => 'githooks.checkcommitauthor.match',
        '(<a.uthor@site>|An Other <an.other@si.te>)');

check_can_commit('This author can commit (2): pattern', 'Dummy commit message',
        'A UThor <a.uthor@site>');

check_can_commit('This author can commit (3): pattern', 'Dummy commit message',
        'An Other <an.other@si.te>');

check_cannot_commit('This author cannot commit (1): pattern',
        qr/commit author \'AnOther/, 'Dummy commit message',
        'AnOther <an.other@si.te>');

$repo->command(config => 'githooks.checkcommitauthor.match',
        '!(<.*\@wrong.company>|<.*\@bad.company>)');

check_cannot_commit('This author cannot commit (2): pattern',
        qr/commit author \'An.* Other.*SHOULD.* NOT/, 'Dummy commit message',
        'An Other <an.other@wrong.company>');

check_cannot_commit('This author cannot commit (3): pattern',
        qr/commit author \'One.* An.* Other.*SHOULD.* NOT/, 'Dummy commit message',
...
$repo->command(config => 'githooks.checkcommitauthor.allow-mailmap-aliases', '1');
$repo->command(config => 'githooks.checkcommitauthor.match-mailmap-name', '1');
...
check_can_commit('This author can commit (1): mailmap(name)', 'Dummy commit message',
        'Proper Committer <proper.committer@company.com>');

# Match with name, if name exists in mailmap.
$repo->command(config => 'githooks.checkcommitauthor.match-mailmap-name','0');
check_can_commit('This author can commit (2): mailmap (no name)',
    'Dummy commit message', 'With Wrong Name <i.alone@any.where>');
$repo->command(config => 'githooks.checkcommitauthor.match-mailmap-name', '1');
check_cannot_commit('This author cannot commit (3): mailmap(no name),ask with name.',
        undef, 'Dummy commit message',
        'With Wrong Name <i.alone@any.where>'); # This test is sort of crazy,
...
$repo->command(config => 'githooks.checkcommitauthor.match-mailmap-name','0');
$repo->command(config => 'githooks.checkcommitauthor.allow-mailmap-aliases', '1');
check_can_commit('This author can commit (4): mailmap', 'Dummy commit message',
        'Al Other <al.other@pomp.pom>');

$repo->command(config => 'githooks.checkcommitauthor.match-mailmap-name','1');
$repo->command(config => 'githooks.checkcommitauthor.allow-mailmap-aliases', '0');
check_cannot_commit('This author cannot commit (2): mailmap',
        undef, 'Dummy commit message',
        'Al Other <al.other@pomp.pom>');
