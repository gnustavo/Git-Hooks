...
If you have hooks invoking external commands or making network connections
(e.g. CheckJira) then it's advisable to set a timeout (C<githooks.timeout = 30>)
in seconds so that the hooks are aborted if they run for too long.
